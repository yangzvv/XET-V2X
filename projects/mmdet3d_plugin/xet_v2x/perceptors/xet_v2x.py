...
class XETV2X(MVXTwoStageDetector):
    def __init__(
        self,
        # pointcloud
        use_pc_veh=True,
        ...
        mm_sca_lidar_first=False,
        train_cfg=None,
        test_cfg=None,
        pretrained=None,
        **kwargs,
        ):
        super(XETV2X, self).__init__(
    ...
    def forward_dummy(
        ...
        **kwargs
        ...
        """
        Forward path for FLOPs/profiling tools.
        It dynamically constructs metadata for the active modalities.
        """
        import numpy as np
        device = next(self.parameters()).device

        img = kwargs.get('img', None)
        points_veh = kwargs.get('points_veh', None)
        points_inf = kwargs.get('points_inf', None)

        # Support MMCV get_flops.py where only args[0] is provided.
        if len(args) == 1:
            x = args[0]
            if x.dim() in [2, 3] and x.shape[-1] >= 4:
                if getattr(self, 'use_pc_veh', False):
                    points_veh = x
                else:
                    points_inf = x
            elif x.dim() in [4, 5]:
        ...
        num_cams = 0
        if getattr(self, 'use_img', False):
            try:
                expected_cams = self.pts_bbox_head.transformer.encoder.num_cams
            except AttributeError:
                expected_cams = 2

            if img is None:
                img = torch.randn(1, expected_cams, 3, 1088, 1920, device=device)
            elif img.dim() == 4:
                img = img.unsqueeze(1)
                if expected_cams == 2:
        ...
        if getattr(self, 'use_pc_veh', False):
            if points_veh is None:
                points_veh = [torch.randn(30805, 5, device=device)]
            elif isinstance(points_veh, torch.Tensor):
                points_veh = [points_veh[0]] if points_veh.dim() == 3 else [points_veh]
            pc.append(points_veh)
            num_lidars += 1

        if getattr(self, 'use_pc_inf', False):
            if points_inf is None and 'points_inf' not in kwargs:
                points_inf = [torch.randn(45339, 5, device=device)]

            if points_inf is not None:
                if isinstance(points_inf, torch.Tensor):
                    points_inf = [points_inf[0]] if points_inf.dim() == 3 else [points_inf]
                pc.append(points_inf)
                num_lidars += 1
        ...
        base_meta = {
            ...
            'next_idx': '000001',
            'box_mode_3d': 0,
            'box_type_3d': getattr(self.bbox_coder, 'box_type_3d', None),
            'can_bus': np.zeros(18, dtype=np.float32),
            'timestamps': [0.0, 0.0],
            'scale_factor': 1.0,
        }

        img_metas, pc_metas = None, None

        if getattr(self, 'use_img', False):
            meta_img = base_meta.copy()
            meta_img['filename'] = [f'image/dummy_cam_{i}.jpg' for i in range(num_cams)]
            meta_img['lidar2img'] = [np.eye(4, dtype=np.float32) for _ in range(num_cams)]
            meta_img['img_shape'] = [(1088, 1920, 3) for _ in range(num_cams)]
            meta_img['pad_shape'] = [(1088, 1920, 3) for _ in range(num_cams)]
            meta_img['ori_shape'] = [(1088, 1920, 3) for _ in range(num_cams)]
            img_metas = [meta_img]

        if pc is not None:
            meta_pc = base_meta.copy()
            meta_pc['filename'] = [f'velodyne/dummy_{i}.pcd' for i in range(num_lidars)]
            meta_pc['lidar2img'] = [np.eye(4, dtype=np.float32) for _ in range(num_lidars)]
            meta_pc['lidar2pseudoimg'] = [np.eye(4, dtype=np.float32) for _ in range(num_lidars)]
            meta_pc['img_shape'] = [(102.4, 102.4, 8) for _ in range(num_lidars)]
            meta_pc['img_ori_shape'] = [(102.4, 102.4, 8) for _ in range(num_lidars)]
            meta_pc['pad_shape'] = [(102.4, 102.4, 8) for _ in range(num_lidars)]
            meta_pc['pts_filename'] = 'velodyne/dummy.bin'
            pc_metas = [meta_pc]
        ...
        if getattr(self, 'use_img', False):
            ...
            used_metas = [base_meta]

        ori_video_mode = getattr(self, 'video_test_mode', False)
        self.video_test_mode = False

        bev_embed, bev_pos = self.get_bevs(
            img=img if getattr(self, 'use_img', False) else None,
            img_metas=img_metas,
            pc=pc,
            pc_metas=pc_metas,
            prev_bev=None
        ...
        self.video_test_mode = ori_video_mode
