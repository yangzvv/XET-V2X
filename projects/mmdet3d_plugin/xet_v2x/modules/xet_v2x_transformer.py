...
from .v2x_spatial_cross_attention import MSDeformableAttention3D
from .decoder import CustomMSDeformableAttention
from mmcv.runner import force_fp32, auto_fp16
from .transformer import PerceptionTransformer
@TRANSFORMER.register_module()
class XETV2XPerceptionTransformer(BaseModule):
    def __init__(self,
                 ...
                 use_lidars_embeds=False,
                 use_multimodal_embeds=False,
                 mm_sca_lidar_first=False,
                 rotate_center=[100, 100],
                 **kwargs):
        super(XETV2XPerceptionTransformer, self).__init__(**kwargs)
        ...
        self.use_lidars_embeds = use_lidars_embeds
        self.use_multimodal_embeds=use_multimodal_embeds
    ...
    def init_layers(self):
        ...
        if self.use_multimodal_embeds:
            if self.mm_sca_lidar_first:
                self.cams_embeds = nn.ParameterList([
                    nn.Parameter(torch.Tensor(self.num_lidars, self.embed_dims)),
                    nn.Parameter(torch.Tensor(self.num_cams, self.embed_dims)),
                ])
            else:
                self.cams_embeds = nn.ParameterList([
                    nn.Parameter(torch.Tensor(self.num_cams, self.embed_dims)),
        ...
        elif self.use_cams_embeds:
            self.cams_embeds = nn.ParameterList([
                nn.Parameter(torch.Tensor(self.num_cams, self.embed_dims))
            ])
        elif self.use_lidars_embeds:
            self.cams_embeds = nn.ParameterList([
                nn.Parameter(torch.Tensor(self.num_lidars, self.embed_dims))
            ])
    ...
    def get_bev_features(
        ...
        for i_mdl in range(mdls):
            ...
            for lvl, feat in enumerate(mmdl_mlvl_feats[i_mdl]):
                ...
                if self.use_multimodal_embeds or self.use_cams_embeds or self.use_lidars_embeds:
                    feat = feat + self.cams_embeds[i_mdl][:, None, None, :].to(feat.dtype)
                feat = feat + self.level_embeds[None, None, lvl:lvl + 1, :].to(feat.dtype)
                if self.use_timestamps:
                    # feat = feat + self.timestamps_mlp(timestamps)[0, None, None, 1].to(feat.dtype) # only veh side
                    feat = feat + self.timestamps_mlp(timestamps)[:, None, None, :].to(feat.dtype) # veh + inf side
